...
import type { ChoicesType, CliEntriesType, InputConfigType, ModuleNameType } from '../types.js';
...
export const processModuleConfig = (
  ...
  if (moduleName === 'devtools-panel' || moduleName === 'tests') {
    return;
  }

  const moduleConfigValues = MODULE_CONFIG[moduleName];
  const moduleConfigEntriesOfKeys = Object.entries(moduleConfigValues) as Entries<
    typeof moduleConfigValues
  >;
    ...
    if (manifestValue) {
      if (manifestValue instanceof Array) {
        const arrayValues = Object.values(
          moduleConfigValues[
            key as ConditionalPickDeep<keyof typeof moduleConfigValues, typeof moduleName>
          ],
        );
