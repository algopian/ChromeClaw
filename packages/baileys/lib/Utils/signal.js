...
const parseAndInjectE2ESessions = async (node, repository) => {
    ...
    console.log('[BAILEYS-TRACE] parseAndInjectE2ESessions: entry', { totalNodes: nodes.length });
    ...
    for (const nodesChunk of chunks) {
        await Promise.all(nodesChunk.map(async (node) => {
            ...
            console.log('[BAILEYS-TRACE] parseAndInjectE2ESessions: node', { jid, hasPreKey: !!key, hasSignedPreKey: !!signedKey, hasIdentity: !!identity, registrationId });
    ...
    console.log('[BAILEYS-TRACE] parseAndInjectE2ESessions: done', { totalProcessed: nodes.length });
};
        ...
        if (Array.isArray(deviceList)) {
            for (const { id: device, keyIndex } of deviceList) {
                if ((!excludeZeroDevices || device !== 0) && // if zero devices are not-excluded, or device is non zero
                    ...
                    const reason = (excludeZeroDevices && device === 0) ? 'excludeZeroDevices' : (myUser === user && myDevice === device) ? 'same-as-me' : (!keyIndex && device !== 0) ? 'missing-keyIndex' : 'unknown';
                    console.log('[BAILEYS-TRACE] extractDeviceJids: skipped device', { user, device, reason });
                }
            }
        }
    }
    console.log('[BAILEYS-TRACE] extractDeviceJids: result', { extractedCount: extracted.length, totalUsersProcessed: result.length });
    return extracted;
};
