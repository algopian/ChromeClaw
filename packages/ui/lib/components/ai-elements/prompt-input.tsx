...
/** Local stub for ai SDK's FileUIPart */
type FileUIPart = {
  type: 'file';
  url?: string;
  mediaType?: string;
...
/** Local stub for ai SDK's ChatStatus */
  ...
  return (
    <PromptInputHoverCard>
      <HoverCardTrigger asChild>
        <div
          className={cn(
            'border-border hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50 group relative flex h-8 cursor-pointer select-none items-center gap-1.5 rounded-md border px-1.5 text-sm font-medium transition-all',
            className,
          )}
          key={data.id}
          {...props}>
