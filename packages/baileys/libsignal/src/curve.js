...
exports.generateKeyPair = function() {
    try {
        console.log('[CURVE-DEBUG] generateKeyPair: trying Node.js native x25519');
        ...
        console.log('[CURVE-DEBUG] generateKeyPair: Node.js native SUCCESS', {
            pubKeyLen: pubKey.length,
            privKeyLen: privKey.length,
        });
        return {
        ...
        console.log('[CURVE-DEBUG] generateKeyPair: FALLBACK to curve25519-js', {
            error: e?.message || String(e),
            hasRandomBytes: typeof nodeCrypto.randomBytes === 'function',
        });
...
exports.calculateAgreement = function(pubKey, privKey) {
    ...
    const hasDH = typeof nodeCrypto.diffieHellman === 'function';
    const hasCreatePrivKey = typeof nodeCrypto.createPrivateKey === 'function';
    const hasCreatePubKey = typeof nodeCrypto.createPublicKey === 'function';
    console.log('[CURVE-DEBUG] calculateAgreement: checking crypto capabilities', {
        hasDiffieHellman: hasDH,
        hasCreatePrivateKey: hasCreatePrivKey,
        hasCreatePublicKey: hasCreatePubKey,
        pubKeyLen: pubKey.byteLength,
        privKeyLen: privKey.byteLength,
    });
    ...
    if(hasDH) {
        try {
            ...
            const result = nodeCrypto.diffieHellman({
                privateKey: nodePrivateKey,
                publicKey: nodePublicKey,
            });
            console.log('[CURVE-DEBUG] calculateAgreement: Node.js native ECDH SUCCESS', {
                resultLen: result.byteLength,
                resultPrefix: result.slice(0, 4).toString('hex'),
            });
            return result;
        } catch(dhErr) {
            console.error('[CURVE-DEBUG] calculateAgreement: Node.js native ECDH FAILED, falling back to curve25519-js', {
                error: dhErr?.message || String(dhErr),
            });
            const secret = curveJs.sharedKey(privKey, pubKey);
            return Buffer.from(secret);
        }
    } else {
        console.log('[CURVE-DEBUG] calculateAgreement: using curve25519-js (no native DH)');
        const secret = curveJs.sharedKey(privKey, pubKey);
        return Buffer.from(secret);
    }
};
