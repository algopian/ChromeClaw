...
const makeMessagesRecvSocket = (config) => {
    ...
    const willSendMessageAgain = (id, participant) => {
        ...
        const willRetry = retryCount < maxMsgRetryCount;
        console.log('[BAILEYS-TRACE] willSendMessageAgain', { id, participant, retryCount, maxMsgRetryCount, willRetry });
        return willRetry;
    };
    ...
    const sendMessagesAgain = async (key, ids, retryNode) => {
        var _a;
        console.log('[BAILEYS-TRACE] sendMessagesAgain: entry', { remoteJid: key.remoteJid, participant: key.participant, ids, retryCount: retryNode?.attrs?.count });
        const msgs = await Promise.all(ids.map(id => getMessage({ ...key, id })));
        for (let i = 0; i < ids.length; i++) {
            const msg = msgs[i];
            console.log('[BAILEYS-TRACE] sendMessagesAgain: getMessage result', { id: ids[i], found: !!msg, protoKeys: msg ? Object.keys(msg) : null });
        }
        ...
        const isGroup = (0, WABinary_1.isJidGroup)(remoteJid);
        console.log('[BAILEYS-TRACE] sendMessagesAgain: sessions asserted', { participant, sendToAll, isGroup });
        if (isGroup) {
        ...
        for (const [i, msg] of msgs.entries()) {
            if (msg) {
                ...
                console.log('[BAILEYS-TRACE] sendMessagesAgain: relaying', { id: ids[i], remoteJid: key.remoteJid, sendToAll, participant: msgRelayOpts.participant });
                await relayMessage(key.remoteJid, msg, msgRelayOpts);
            }
            else {
                console.log('[BAILEYS-TRACE] sendMessagesAgain: WARNING getMessage returned undefined', { remoteJid: key.remoteJid, id: ids[i] });
            ...
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (attrs.type === 'retry') {
                        ...
                        console.log('[BAILEYS-TRACE] handleReceipt: retry detected', { msgId: attrs.id, remoteJid, participant: key.participant, fromMe: key.fromMe, retryCount: retryNode?.attrs?.count, allIds: ids });
