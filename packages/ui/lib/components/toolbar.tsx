...
import { ArrowUpIcon, SquareIcon, BookOpenIcon } from 'lucide-react';
...
type ToolProps = {
  ...
  onClick: (context: { sendMessage: SendMessageFn }) => void;
};
    ...
    <Tooltip open={isHovered && !isAnimating}>
      <TooltipTrigger asChild>
        <motion.div
          ...
          whileTap={{ scale: 0.95 }}>
          {selectedTool === description ? <ArrowUpIcon className="size-4" /> : icon}
        </motion.div>
      </TooltipTrigger>
      <TooltipContent
...
const ReadingLevelSelector = ({
  ...
  const LEVELS = [
    t('toolbar_elementary'),
    t('toolbar_middleSchool'),
    t('toolbar_keepLevel'),
    t('toolbar_highSchool'),
    t('toolbar_college'),
    t('toolbar_graduate'),
  ];
                ...
                if (currentLevel !== 2 && hasUserSelectedLevel) {
                  sendMessage({
                    ...
                    parts: [
                      {
                        type: 'text',
                        text: t('toolbar_adjustReading', LEVELS[currentLevel]),
                      },
                    ],
                  });
...
type SetMessagesFn = React.Dispatch<React.SetStateAction<any[]>>;
