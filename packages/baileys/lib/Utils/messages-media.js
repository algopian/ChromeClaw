...
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (stream, { mediaType, fileEncSha256B64, timeoutMs }) => {
        var _a, _b;
        // Convert stream to Buffer for browser compatibility — axios in browser
        // (XMLHttpRequest) cannot POST a Node.js Readable stream directly.
        const uploadBuffer = await (0, exports.toBuffer)(stream);
        ...
        for (const { hostname } of hosts) {
            ...
            let result;
            try {
                const body = await axios_1.default.post(url, uploadBuffer, {
                    ...options,
                    headers: {
                        ...options.headers || {},
